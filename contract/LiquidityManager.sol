...
contract LiquidityManager is Ownable{
    ...
    address NftwToken;
    address pair;
    IdexRouter02 router02 = IdexRouter02();
    
    
    function init(address _NftwToken, address _USDT, address _router02, address _pair) public onlyOwner() {
        NftwToken = _NftwToken;
        USDT = _USDT;
        pair = _pair;
        router02 = IdexRouter02(_router02);
        IERC20(NftwToken).approve(address(router02), uint256(-1));
    ...
    function addLiquidity(uint256 _amountADesired, uint256 _amountBDesired) public {
        ...
        IERC20(NftwToken).transferFrom(msg.sender, address(this), _amountBDesired);
        router02.addLiquidity(USDT, NftwToken, _amountADesired, _amountBDesired, 0, 0, msg.sender, block.timestamp);
        
    }

    function removeLiquidity(uint256 _liquidity) public {
        IERC20(pair).transferFrom(msg.sender, address(this), _liquidity);
        router02.removeLiquidity(USDT, NftwToken, _liquidity, 0, 0, msg.sender, block.timestamp);
    }
